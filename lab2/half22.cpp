...
double half__interval(double a, double b, double fa, double fb) {
    double root (0.);
    total__iterations = 0.;
    root = (fa < 0. && fb > 0. ? __MAB__try(a, b) : fa > 0. && fb < 0. ? __MAB__try(b, a) : (b + 1.));
    newline();
    return root;
}
double __MAB__try(double neg__point, double pos__point) {
    ...
    return (close__enough_Q(neg__point, pos__point) ? midpoint : (test__value = f(midpoint), display("+"), total__iterations = total__iterations + 1., test__value > 0. ? __MAB__try(neg__point, midpoint) : (test__value < 0. ? __MAB__try(midpoint, pos__point) : midpoint)));
}
bool close__enough_Q(double x, double y) {
...
double f(double z) {
    return sin(z - 0.25 * pi) + log(z) - 0.8;
}
int main() {
    display("Variant 208-16\n");
    display(root(5.3, 6.0));
    newline();
