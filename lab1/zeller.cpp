...
extern double yyyy;
double day__of__week() {
    return zeller(dd, 8. + (mm <= 2. ? mm + 2. : mm - 10.), remainder(1. + (mm < 3. ? yyyy - 2. : (yyyy - 1.)), 100.), quotient((2. >= mm ? yyyy - 1 : true ? yyyy : _infinity), 100.));
}
double zeller(double d, double m, double y, double c) {
...
double birthday(double dw) {
    ...
    display((dw == 0. ? "Sunday " : dw == 1. ? "Monday " : dw == 2. ? "Tuesday " : dw == 3. ? "Wednesday " : dw == 4. ? "Thursday " : dw == 5. ? "Friday " : "Saturday "));
    display(dd);
    display(".");
    display(mm);
    display(".");
